...
def fetch_live_catalog(root: Path) -> dict[str, Any]:
    ...
    req = urllib.request.Request(
        url,
        headers={
            "User-Agent": "mi.do.ri-preview-generator/4.1.0",
...
def product_page(product: dict[str, Any], image_rel: str, has_product_photo: bool) -> str:
    ...
    alt = f"{name} — {brand}" if has_product_photo else "mi.do.ri — Multibrand Muslim Fashion"
    desc = f"Lihat {name} dari {brand} di mi.do.ri Multibrand Muslim Fashion. Konfirmasi stok melalui WhatsApp."
